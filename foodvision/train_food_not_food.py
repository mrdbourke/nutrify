...
parser = argparse.ArgumentParser(description="Train a Food Not Food model.")
...
group.add_argument(
    ...
    help="model to use (default: 'efficientnet_b0')",
)
group.add_argument(
...
group.add_argument(
    "--wandb-job-type",
    default="train and track food not food model",
    help="Weights & Biases job type",
)
group.add_argument(
    "--wandb-dataset-artifact",
    default="food_not_food_images:latest",
    help="Weights & Biases dataset artifact name",
)
group.add_argument(
    "--wandb-labels-artifact",
    default="food_not_food_annotations:latest",
...
run = wandb.init(
    ...
    tags=["training_food_not_food_model"], # TODO set this on args
...
def log_model_artifact_to_wandb(model_path, project=args.wandb_project, run=run):
    ...
    model_artifact = wandb.Artifact(
        name="trained_model",
        type="food_not_food_model",
        description="A food_not_food model.",
    )
